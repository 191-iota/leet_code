...
public class InsertInterval {
  ...
  private static int[][] insert5(int[][] intervals, int[] newInterval) {
    ...
    for (int i = 0; i < intervals.length; i++) {
      ...
      if (nextSmallest < newInterval[1] && newInterval[1] < intervals[i][1]) {
        ...
        i--;
        checkMerge = false;
      } else if (newInterval[1] <= nextSmallest && newInterval[1] >= intervals[i][1]
          || i == intervals.length - 1 && newInterval[1] > intervals[i][1]) {
        resultList.put(toMerge, newInterval[1]);
        checkMerge = false;
      } else if (newInterval[1] <= nextSmallest && newInterval[1] <= intervals[i][1]
          || i == intervals.length - 1 && newInterval[1] < intervals[i][1]) {
  ...
  private static int[][] insert4(int[][] intervals, int[] newInterval) {
    ...
    for (int i = 1; i < intervals.length; i++) {
      ...
      if (result.lowerEntry(intervals[i][0]) != null
          ...
          && !result.containsKey(newInterval[0])) {
  ...
  // if the last number is not part of it or the same as the next interval, then skip this when
  // building an array
  private static int[][] insert3(int[][] intervals, int[] newInterval) {
    ...
    for (int i = 0; i < intervals.length - 1; i++) {
      ...
      // case 2.1: If new interval ending is bigger than current ending but smaller than next
      // beginning
      if (firstIntervalSet
