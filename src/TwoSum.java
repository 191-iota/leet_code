...
public class TwoSum {
    ...
    // least amount of values => check for bigger sums <= target, zero target => find number that has the equivalent absolute value and negative expression
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (Math.abs(currentSum) + Math.abs(nums[i]) <= Math.abs(target) || target == 0 && i == 0) {
                ...
                for (int j = i + 1; j < nums.length; j++) {
                    if (Math.abs(nums[i]) < Math.abs(nums[j]) && Math.abs(currentSum) + Math.abs(nums[j]) <= Math.abs(target)) {
                        map.put(j, nums[j]);
                        currentSum += nums[j];
                    }
                }
